#include "turn_on_mercury_robot/mercury_robot.h"
#include "rclcpp/rclcpp.hpp"
#include "turn_on_mercury_robot/Quaternion_Solution.h"
#include "ackermann_msgs/msg/ackermann_drive_stamped.hpp"     // CHANGE
#include "mercury_robot_msg/msg/data.hpp"     // CHANGE
...
void turn_on_robot::Publish_Odom()
{
    ...
    mercury_robot_msg::msg::Data robotpose;
    mercury_robot_msg::msg::Data robotvel;
...
: rclcpp::Node ("mercury_robot")
{
  ...
  robotpose_publisher = create_publisher<mercury_robot_msg::msg::Data>("robotpose", 10);
  //robotpose_timer = create_wall_timer(1s/50, [=]() { Publish_Odom(); });

  robotvel_publisher = create_publisher<mercury_robot_msg::msg::Data>("robotvel", 10);
  ...
  {
    RCLCPP_ERROR(this->get_logger(),"mercury_robot can not open serial port,Please check the serial port cable! "); //If opening the serial port fails, an error message is printed //如果开启串口失败，打印错误信息
  }
  if(Stm32_Serial.isOpen())
  {
    RCLCPP_INFO(this->get_logger(),"mercury_robot serial port opened"); //Serial port opened successfully //串口开启成功提示
  }
}
