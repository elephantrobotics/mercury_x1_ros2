...
using namespace std;
class mercury_joy : public rclcpp::Node
{
...
: Node ("mercury_joy")
{
    RCLCPP_INFO(this->get_logger(),"mercury_joy111");
   //读取参数服务器中的变量值
     flag_mec=0;
   ...
   this->get_parameter("v_angular", v_angular);
    ...
    sub =  this->create_subscription<sensor_msgs::msg::Joy>("/joy", 10, std::bind(&mercury_joy::callback, this, _1)); //订阅手柄发来的数据
...
int main(int argc, char * argv[]){
    ...
    printf("joy_control\n");
    rclcpp::spin(std::make_shared<mercury_joy>());
    return 0;
}
